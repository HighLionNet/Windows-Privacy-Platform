...
The main shell exposes an identity-first Dashboard, grouped outcome conflicts, category-first editable privacy/security/Windows domains, Knowledge Explorer, System Explorer, Services diagnosis, search, App Settings, and About. Navigation has history, breadcrumbs, and persistent labeled search/filter controls. Four token-driven themes are selectable at runtime. Search and Dashboard results land on category lists with highlighted setting bars; detail is explicit. Product and build identity comes from assembly metadata.
...
Service start/stop/restart uses a separate typed `ServiceController` route after Administrator authorization, explicit confirmation, and `ServiceMutationPolicy` approval. Critical names, Microsoft/unknown publishers, boot/system starts, shared svchost groups, and incomplete evidence are permanently diagnosis-only. Scheduled tasks, packages, Windows features, capabilities, arbitrary firewall rules, and live dynamic inventory have no backend route.
