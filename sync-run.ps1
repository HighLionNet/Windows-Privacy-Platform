...
.DESCRIPTION
  1. Stops any running WindowsPrivacyPlatform processes
  2. Fetches origin/main and hard-resets the repo to match GitHub
  3. Cleans previous build outputs
  4. Publishes a fresh win-x64 Release build into .\run\
  5. Verifies the binary exists and starts it

  Fixed launch path (use this for shortcuts):
    <repo-root>\run\WindowsPrivacyPlatform.exe

  Default repo root for this project:
    C:\Windows Privacy Platform
...
try {
    ...
    if (-not $KeepLocalChanges) {
        # Discard local modifications so the tree matches GitHub exactly.
...
# Clean bin/obj under Source to force a real rebuild
Get-ChildItem -Path $SourceDir -Directory -Recurse -ErrorAction SilentlyContinue |
    Where-Object { $_.Name -in @("bin", "obj") } |
    ForEach-Object {
        Remove-Item -LiteralPath $_.FullName -Recurse -Force -ErrorAction SilentlyContinue
    }
Write-Ok "Cleaned bin/obj and run\"

# 4) Publish Release win-x64 into .\run
Write-Step "Publish Release (win-x64) → run\"
...
# Optional: show assembly informational version if available
try {
