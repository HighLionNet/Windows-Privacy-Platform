...
The current product on `main` (today 2.6.2) receives security fixes. Older `vX.Y.Z` branches are snapshots. Upgrade before reporting product behavior, because catalog and write-authorization rules evolve together.
...
- Dynamic inventory and arbitrary firewall rules never receive a `WritableTarget`.
...
- `PolicyChangeService` accepts verified registry targets only. A separate `InventoryChangeService` permits only optional non-Microsoft service start/stop/restart and non-Microsoft scheduled-task enable/disable from a fresh exact snapshot row, after confirmation and with live read-back. It cannot change service start type or task definitions.
...
- Unsigned executable hash changes are status-only. Authenticode-signed builds must validate to the HighLionNet publisher before high-impact Settings are enabled; catalog authorization integrity remains an independent interlock.
