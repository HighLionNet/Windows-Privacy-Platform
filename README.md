...
## What the current source line delivers
...
- No scheduled-task, package, optional-feature, or arbitrary firewall-rule mutation. Service runtime control is limited to verified non-Microsoft optional services; startup configuration is never changed.
...
4. Open `WindowsPrivacyPlatform.exe` and choose **View-only** or **Admin**. Admin mode requests Windows elevation for machine policy.
...
Every writable object is deny-by-default. Discovery data never grants mutation rights. A change must have a complete typed target in a source-controlled allowlist, match that compiled target at apply time, and follow:

```text
pre-read → explicit confirmation → typed operation → independent read-back → local audit
```

Failure at any stage is reported as failure; a textual match with the wrong registry type is not accepted. System Explorer stays read-only. See [Status/Safety_Model.md](Status/Safety_Model.md) and [Status/Architecture.md](Status/Architecture.md). Standing engineering constraints: [Status/AI_Handoff.md](Status/AI_Handoff.md).
