...
internal static class CatalogExpansion
{
    internal static IReadOnlyList<ManagedObject> CreateCoverageBatch()
    {
        ...
        list.Add(Pol("policy.bitlocker.requiredeviceencryption", "Require Device Encryption", "Requires BitLocker device encryption on supported Windows editions.", "Encryption begins only when Windows prerequisites and recovery-key safeguards are satisfied.", RiskLevel.High, FeatureCategory.RegistryPolicy, ComponentOwner.Other, ControlLevel.AdministratorControlled, ProductDomain.BitLocker, "BitLocker", @"HKLM\SOFTWARE\Microsoft\PolicyManager\current\device\BitLocker\RequireDeviceEncryption"));
