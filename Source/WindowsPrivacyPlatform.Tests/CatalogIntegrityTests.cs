...
public class CatalogIntegrityTests
{
    ...
    [Fact]
    public void Public_settings_are_exact_typed_write_contracts()
    {
        ...
        Assert.Contains(settings, item => item.ProductDomain == ProductDomain.WindowsUpdate);
        Assert.Contains(settings, item => item.ProductDomain == ProductDomain.Storage);
    }
